...
export default {
  ...
  getMatch(id){
    return new Promise((resolve, reject)=>{
      var url="http://api.sportradar.us/dota2-t1/en/matches/"+id+"/summary.json?api_key="+config.api_key;
      superAgent
      .get(url)
      .then((response)=>{
        resolve(response.body);
  ...
  getTourSchedule(id){
    return new Promise((resolve, reject)=>{
     var url = "http://api.sportradar.us/dota2-t1/en/tournaments/" + id + "/schedule.json?api_key=" + config.api_key;
      superAgent
      .get(url)
      .then((response)=>{
        resolve(response.body);
